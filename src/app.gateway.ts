    ...
    implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
    ...
    private connectedUsers: WeakMap<Socket, OnlineUser> = new WeakMap<
        Socket,
        OnlineUser
    >();
    private userConnections: Map<OnlineUser['id'], Set<Socket>> = new Map<
        OnlineUser['id'],
        Set<Socket>
    >();
    private debouncedSmileStoppers: Map<Socket, () => void> = new Map<
        Socket,
        () => void
    >();
    ...
    afterInit(server: any): void {
        this.logger.log(
            `AppGateway initialized on port ${process.env.WS_PORT}`,
        );
    }
    ...
    async handleDisconnect(client: Socket): Promise<void> {
        try {
            ...
            const collocatorIds = await this.chatsService.getUserCollocators(
                onlineUser.id,
            );
            collocatorIds.forEach((collocatorId: string) => {
                const collocatorClients: Set<Socket> | undefined =
                    this.userConnections.get(collocatorId);
                collocatorClients?.forEach((collocatorClient: Socket): void => {
                    if (collocatorClients.size === 1) {
                        collocatorClient.emit(
                            'userOffline',
                            this.connectedUsers.get(client),
                        );
                    }
                });
            });
        ...
        try {
            ...
            collocatorIds.forEach((collocatorId: string) => {
                ...
                collocatorClients?.forEach((collocatorClient: Socket): void => {
                    client.emit(
                        'userOnline',
                        this.connectedUsers.get(collocatorClient),
                    );
                    collocatorClient.emit(
                        'userOnline',
                        this.connectedUsers.get(client),
    ...
    async handleChats(client: Socket): Promise<void> {
        try {
            ...
            chatIds.forEach(async (chatId: string) => {
                ...
                const userId = await this.chatsService.getChatCollocator(
                    chatId,
                    onlineUser.id,
                );
    ...
    async handleMessage(
        ...
        try {
            const onlineUser = this.connectedUsers.get(client);
            const collocatorClients = this.userConnections.get(
                messageInfo.toUserId,
            );
            const userClients = this.userConnections.get(onlineUser.id);
            const chatId = await this.chatsService.getUsersChat([
                onlineUser.id,
                messageInfo.toUserId,
            ]);
            let chat: Chat;
    ...
    async handleReadMessage(client: Socket, messageId: string): Promise<void> {
        try {
            ...
            const chatId = await this.messagesService.getChatIdByMessageId(
                messageId,
            );
            const collocatorId = await this.chatsService.getChatCollocator(
                chatId,
                onlineUser.id,
            );
            ...
            if (!debouncedSmileStopper) {
                debouncedSmileStopper = debounce(() => {
                    ...
                    collocatorIds.forEach((collocatorId: string) => {
                        const collocatorClients: Set<Socket> | undefined =
                            this.userConnections.get(collocatorId);
                        collocatorClients?.forEach(
                            (collocatorClient: Socket): void => {
                                collocatorClient.emit(
                                    'userOnline',
                                    this.connectedUsers.get(client),
                                );
